const estudantes = [
  ...
  const msgbotProjeto = (array) => {
    const pessoaParamandarMsg = array.find((pessoa) => pessoa.status < 80)
    if (pessoaParamandarMsg) {
      const nomePessoa = pessoaParamandarMsg.nome;
      console.log(`Olá ${nomePessoa}, o prazo para entrega do projeto é daqui a 3 dias, vc precisa de alguma ajuda?`)
    }
  }
  ...
  const msgbotPedidoAjuda = (array) => {
    const pessoaParamandarMsg = array.find((pessoa) => pessoa.status == 100)
    if (pessoaParamandarMsg) {
      const nomePessoa = pessoaParamandarMsg.nome;
      console.log(`Olá ${nomePessoa}, parabéns por ter finalizado 100% do projeto, o que vc acha de ajudar outras pessoas da turma a tbm entregarem o projeto?`)
    }
  }
